...
static max31865_err_t read_register_8(max31865_t *device, uint8_t reg, uint8_t *data)
{
    ...
    MAX31865_ERROR_CHECK(device->platform->spi_read(device, reg, 1, data));
...
static max31865_err_t write_register_8(max31865_t *device, uint8_t reg, const uint8_t data)
{
    ...
    MAX31865_ERROR_CHECK(device->platform->spi_write(device, reg, 1, &data));
...
static max31865_err_t read_register_16(max31865_t *device, uint8_t reg, uint16_t *data)
{
    ...
    uint8_t buffer[2];
    MAX31865_ERROR_CHECK(device->platform->spi_read(device, reg, 2, buffer));
...
static max31865_err_t read_register_32(max31865_t *device, uint8_t reg, uint32_t *data)
{
    ...
    uint8_t buffer[4];
    MAX31865_ERROR_CHECK(device->platform->spi_read(device, reg, 4, buffer));
...
static max31865_err_t write_register_32(max31865_t *device, uint8_t reg, const uint32_t data)
{
    ...
    MAX31865_ERROR_CHECK(device->platform->spi_write(device, reg, 4, buffer));
    ...
    const uint8_t filter_mode)
{
    ...
    MAX31865_ERROR_CHECK(max31865_clear_fault_status(device));

    // Legge il registro Configuration
    uint8_t reg_val = 0;
    MAX31865_ERROR_CHECK(read_register_8(device, REG_CONFIG, &reg_val));

    // Resetta tutti i bit da configurare
    reg_val &= ~(MASK_CONFIG_WIRE | MASK_CONFIG_FILTER);
    ...
    MAX31865_ERROR_CHECK(write_register_8(device, REG_CONFIG, reg_val));
...
max31865_err_t max31865_read_rtd(max31865_t *device, uint16_t *rtd, uint8_t *fault)
{
    ...
    uint16_t reg_val = 0;
    MAX31865_ERROR_CHECK(read_register_16(device, REG_RTD_MSB, &reg_val));
...
max31865_err_t max31865_get_rtd_threshold(max31865_t *device, uint16_t *lower, uint16_t *upper)
{
    ...
    uint32_t reg_val = 0;
    MAX31865_ERROR_CHECK(read_register_32(device, REG_HIGH_FAULT_MSB, &reg_val));
...
max31865_err_t max31865_set_rtd_threshold(max31865_t *device, uint16_t lower, uint16_t upper)
{
    ...
    MAX31865_ERROR_CHECK(write_register_32(device, REG_HIGH_FAULT_MSB, reg_val));
...
max31865_err_t max31865_read_celsius(max31865_t *device, float *celsius, uint8_t *fault)
{
    ...
    MAX31865_ERROR_CHECK(max31865_rtd_to_celsius(device, reg_val >> 1, &temperature));
...
max31865_err_t max31865_get_celsius_threshold(max31865_t *device, float *lower, float *upper)
{
    ...
    if (lower)
    {
        ...
        // Converte il valore RTD in gradi Celsius
        float celsius = 0.0f;
        MAX31865_ERROR_CHECK(max31865_rtd_to_celsius(device, low_rtd, &celsius));
    ...
    if (upper)
    {
        ...
        // Converte il valore RTD in gradi Celsius
        float celsius = 0.0f;
        MAX31865_ERROR_CHECK(max31865_rtd_to_celsius(device, high_rtd, &celsius));
    ...
    max31865_t *device, const float lower, const float upper)
{
    ...
    MAX31865_ERROR_CHECK(max31865_celsius_to_rtd(device, lower, &lower_rtd));

    MAX31865_ERROR_CHECK(max31865_celsius_to_rtd(device, upper, &upper_rtd));
...
max31865_err_t max31865_stop_fault_detection(max31865_t *device)
{
    ...
    if (reg_val == 0x02)
    {
        ...
        // La X indica che quei bit possono assumere qualsiasi valore.
        MAX31865_ERROR_CHECK(max31865_set_config(device, 0b11101110, 0b10001100));
    }
...
max31865_err_t max31865_get_fault_status(max31865_t *device, uint8_t *fault)
{
    ...
    uint8_t reg_val = 0;
    MAX31865_ERROR_CHECK(read_register_8(device, REG_FAULT_STATUS, &reg_val));
...
max31865_err_t max31865_clear_fault_status(max31865_t *device)
{
    ...
    reg_val &= ~(MASK_CONFIG_ONE_SHOT | MASK_CONFIG_FAULT_DETECTION);
...
max31865_err_t max31865_start_single_conversion(max31865_t *device)
{
    ...
    uint16_t rtd = 0;
    MAX31865_ERROR_CHECK(read_register_16(device, REG_RTD_MSB, &rtd));
